...
/*   Updated: 2024/03/04 17:42:54 by mvisca           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	initialize(t_ms *ms, int ac, char **av, char **envp)
{
	...
	ft_printf(YELLOW"\nQue hay en envp"BLUE" %p envp[0] %p"RESET" %s\n"RESET, envp, envp[0], envp[0]);
	ft_printf(YELLOW"\nQue hay en envp"BLUE" %p envp[1] %p"RESET" %s\n"RESET, envp, envp[1], envp[1]);
	ft_printf(YELLOW"\nQue hay en envp"BLUE" %p envp[2] %p"RESET" %s\n"RESET, envp, envp[2], envp[2]);
	ft_printf(YELLOW"\nQue hay en envp"BLUE" %p envp[31] %p"RESET" %s\n"RESET, envp, envp[31], envp[2]);
